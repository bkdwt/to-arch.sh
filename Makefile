...
to_arch: src/*
	...
	# This line is for single quote escaping in sudo bash -c.
	if command -v gsed; then gsed "s/'/'\"'\"'/g" src/convert.sh > src/convert_.sh; else sed "s/'/'\"'\"'/g" src/convert.sh > src/convert_.sh; fi
